...
if(isset($_POST['save_multiple_data']))
{
    ...
    $open_ended = $_POST['open_ended'];
    $dichotomous = $_POST['dichotomous'];
    $close_ended = $_POST['close_ended'];
    ...
    foreach($open_ended as $open_ended)
    {
      if($open_ended){
        echo '<b>Question ' . $i++ . '</b>.)&nbsp;'.  $open_ended .'<textarea></textarea>'; 
      }
      ?>
      <?php
    }
    
    foreach($dichotomous as $dichotomous)
    {
      if($dichotomous ){
        echo '<b>Question ' . $i++ . '</b>.)&nbsp;'.  $dichotomous . '
        <table>
          <tr>
    ...
    foreach($close_ended as $index => $close_endeds)
    {
      if($close_endeds){
        echo '<b>Question ' . $i++ . '</b>.)&nbsp;'.  $close_endeds . '
        <table>
        <tr>
          <td><input type="checkbox"></td>
          <td class="check_close">'. $answer1[$index] .'</td>
          <td><input class="" type="checkbox"></td>
          <td>'. $answer2[$index] .'</td>
        <tr>
      </table>
      <table>
          <tr>
            <td><input type="checkbox"></td>
            <td class="check_close">'. $answer3[$index] .'</td>
            <td><input class="" type="checkbox"></td>
            <td>'. $answer4[$index] .'</td>
          <tr>
        </table>
...
// Output the generated PDF to Browser   (Always stream it)                       
